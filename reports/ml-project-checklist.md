...
1. [X] Train many quick and dirty models from different categories (e.g., linear, naive, Bayes, SVM, Random Forests, 
neural net, etc.) using standard parameters.  
2. [x] Measure and compare their performance.  
    - For each model, use N-fold cross-validation and compute the mean and standard deviation of their performance. 
3. [x] Analyze the most significant variables for each algorithm.  
4. [x] Analyze the types of errors the models make.  
    - What data would a human have used to avoid these errors?  
5. [x] Have a quick round of feature selection and engineering.  
6. [X] Have one or two more quick iterations of the five previous steps.  
7. [x] Short-list the top three to five most promising models, preferring models that make different types of errors.  
...
1. [x] Fine-tune the hyperparameters using cross-validation.  
...
3. [x] Once you are confident about your final model, measure its performance on the test set to estimate the generalization error.
